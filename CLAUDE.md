...
- **Pinterest**: standart OAuth2 (PKCE YOK; client kimlikleri Basic auth header'ında). `/api/auth/pinterest/start`, `/callback`, `/status`, `/boards`.
  ...
  - **Sandbox board'ları AYRI bir varlık dünyasındadır**: `pinterest.com` arayüzünde elle açılan board `api-sandbox`'ta GÖRÜNMEZ (`GET /boards` 200 + boş liste döner, hata değil). Sandbox'ta board yalnızca `POST /v5/boards` ile yaratılabilir → `lib/pinterest/boards.createBoard` + `/admin` kartındaki "Board oluştur" alanı. Bu yüzden scope'lara `boards:write` dahildir; scope değişikliği **yeniden yetkilendirme** gerektirir (eski token yeni scope'u taşımaz).
