...
- **Pinterest**: standart OAuth2 (PKCE YOK; client kimlikleri Basic auth header'ında). `/api/auth/pinterest/start`, `/callback`, `/status`, `/boards`.
  ...
  - **Sandbox board'ları AYRI bir varlık dünyasındadır**: `pinterest.com` arayüzünde elle açılan board `api-sandbox`'ta GÖRÜNMEZ (`GET /boards` 200 + boş liste döner, hata değil). Sandbox'ta board yalnızca `POST /v5/boards` ile yaratılabilir ve yalnızca `DELETE /v5/boards/{id}` ile silinebilir → `lib/pinterest/boards.createBoard` / `deleteBoard` + `/admin` kartındaki "Board oluştur" alanı ve her board çipinin yanındaki `×`. Bu yüzden scope'lara `boards:write` dahildir; scope değişikliği **yeniden yetkilendirme** gerektirir (eski token yeni scope'u taşımaz).
  - **API route'ları upstream hatası için 502/504 DÖNMEZ** (`UPSTREAM_FAILED = 424`): DigitalOcean/Cloudflare origin'den gelen 502/504'ü kendi **HTML** hata sayfasıyla değiştirir; JSON gövdemiz kaybolur, panel `Unexpected token '<'` verir ve gerçek sebep hiç görünmez. Teşhis ipucu: yanıt başlığındaki `x-do-orig-status` uygulamanın GERÇEK durum kodunu gösterir.
  - **Timeout**: Pinterest çağrıları (`client.pinterestFetch` + `oauth.postToken`) `TIMEOUTS.pinterest` = **30 sn** ile sınırlıdır — diğer servislerden kısa, çünkü panel butonlarının içinde senkron beklenir ve barındırma proxy'si ~60 sn'de kendi **HTML** 504'ünü basar (istemci onu JSON sanıp `Unexpected token '<'` verir). Bütçe proxy sınırının altında kalmalı.
...
- **Dosya formatı**: Baskı (print) dijital ürün dosyaları **5 JPG** (oran başına en büyük boyut): `ratio_2x3` 24x36, `ratio_3x4` 24x32, `ratio_4x5` 24x30, `ratio_11x14` 22x28, `ratio_5x7` 25x35 — hepsi **300 DPI, baseline libjpeg q90 (4:2:0)**, her biri <20MB (aşarsa kalite %5'er düşer, zemin %60). Müşteri açıklamadaki alt boyutları baskıcıda küçülterek alır. ZIP YOK. Frame TV (tv) ürünü **2 JPG**'dir: 3840×2160 + 1920×1080, **300 DPI, q97 4:4:4** (8.3 MP'lik dosyalar 20MB tavanının çok altında kaldığı için kalite yukarı çekilir; ölçüm: 4K 1.56MB→4.09MB).
- **JPG export ayarları ürün tipine bağlıdır** — `productConfig(...).jpeg` (`density`/`quality`/`chromaSubsampling`) tek kaynaktır; `packageJpegs` bunu uygular. **DPI her ürün tipinde 300**: ekran ürününde teknik olarak önemsiz olsa da dosya özelliklerinde "72 DPI" görünmesi alıcıda düşük kalite izlenimi yaratıyor. Baskı tarafındaki q90/4:2:0 ölçülmüş bellek sözleşmesinin parçasıdır (77 MP dosyalar) — yükseltmeden önce §11'deki kaynak sınırlarını oku.
