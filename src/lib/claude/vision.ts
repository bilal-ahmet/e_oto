 ...
 *
 * Referans görsel + (opsiyonel) kullanıcı notu verilir; görsele bakılarak text-to-image modellerine
 * (FLUX / Imagen) gönderilecek SPESİFİK editing talimatı döner. Amaç: kompozisyon/atmosfer korunarak,
 * bazı somut farklarla (şekil/renk/bakış açısı) yeni bir görsel üretmek — birebir kopya değil.
 *
...
const SYSTEM = `You write a single ENGLISH image transformation instruction for a text-to-image model (FLUX / Imagen). You are given a reference image. Produce a precise editing instruction that creates a NEW image clearly inspired by the reference, with a few deliberate differences — NOT a faithful copy.
